...
int initServerNet(int port) {
    ...
    sockfd = accept(listensock, (struct sockaddr*)&serv_addr, &clilen);
...
static void tsappend(const char* msg, char** tags, int newline) {
    GtkTextIter end;
...
static void sendMessage(GtkWidget* w, gpointer d) {
    char* tags[] = {"self", NULL};
    tsappend("me: ", tags, 0);
    GtkTextIter s, e;
    gtk_text_buffer_get_start_iter(mbuf, &s);
    gtk_text_buffer_get_end_iter(mbuf, &e);
    gchar* message = gtk_text_buffer_get_text(mbuf, &s, &e, TRUE);
    size_t len = g_utf8_strlen(message, -1);
    session_send(sess, (const unsigned char*)message, len);
...
int main(int argc, char* argv[]) {
    ...
    char* mykey = NULL;
    char* peerkey = NULL;
    while ((opt = getopt_long(argc, argv, "c:lp:i:e:h", opts, NULL)) != -1) {
        switch(opt) {
            case 'c': strncpy(host, optarg, HOST_NAME_MAX); break;
            case 'l': isclient = 0; break;
            case 'p': port = atoi(optarg); break;
            case 'i': mykey = optarg; break;
            case 'e': peerkey = optarg; break;
            case 'h': printf("Usage: chat [-l] [-c host] [-p port] [-i idfile] [-e peerfile]\n");
                      return 0;
        }
    }
    ...
    if (!sess) { fprintf(stderr, "handshake failed\n"); return 1; }

    gtk_init(&argc, &argv);
    GError* err = NULL;
    GtkBuilder* builder = gtk_builder_new();
    if (!gtk_builder_add_from_file(builder, "layout.ui", &err)) {
        g_printerr("Error loading UI: %s\n", err->message);
        return 1;
    }

    mark = gtk_text_mark_new(NULL, TRUE);
    tview = GTK_TEXT_VIEW(gtk_builder_get_object(builder, "transcript"));
    mbuf  = gtk_text_view_get_buffer(
                GTK_TEXT_VIEW(gtk_builder_get_object(builder, "message"))
            );
    tbuf  = gtk_text_view_get_buffer(tview);

    gtk_builder_connect_signals(builder, NULL);
    GtkWidget* window = GTK_WIDGET(gtk_builder_get_object(builder, "window"));
    GtkWidget* send_btn = GTK_WIDGET(gtk_builder_get_object(builder, "send"));
    g_signal_connect(send_btn, "clicked", G_CALLBACK(sendMessage), NULL);
...
void* recvMsg(void* d) {
    while (1) {
        ...
        size_t len = 0;
        if (session_recv(sess, &buf, &len) != 0) break;
        buf[len] = '\0';
